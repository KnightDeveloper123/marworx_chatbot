...
router.post("/add", middleware, async (req, res) => {
    try {
        const {  channel_name, campaign_name, message_content, sector, template_name, template_type, template_lang, header, body } = req.body;
        // console.log(req.body)
        const insertQuery = 'insert into campaign (channel_name, campaign_name, message_content, sector, template_name, template_type, template_lang, header, body, is_status) values (?, ?, ?, ?, ?, ?, ?, ?, ?, "Sent");'
        connection.execute(insertQuery, [channel_name, campaign_name, message_content, sector, template_name, template_type, template_lang, header, body, ], (err, data) => {
            if (err) {
                console.log(err);
...
router.get('/getAllCampaign', middleware, async(req, res)=>{
    ...
    const data=await executeQuery(`select campaign.*, s.name as sname
        from campaign 
       left join sector as s on campaign.sector=s.id
      where campaign.status=0  order by campaign.id desc`) 
